...
# Instantiate the PGM.
pgm = daft.PGM([6.3, 4.55], origin=[0.3, 0.3])
...
# Latent variable.
pgm.add_node(daft.Node("x_old", r"$(p_0,p_1)$", 1, 2, fixed=True))
pgm.add_node(daft.Node("x", r"$x$", 3, 2))
pgm.add_node(daft.Node("payoff", r"$\Gamma$", 4, 3))

# Yes, this is a stupid way to add more txt, but I don't see how to otherwise!!
pgm.add_node(daft.Node("q1", r'\texttt{"serious" action}', 4, 4.5,plot_params={"ec": "none"}))
pgm.add_node(daft.Node("q2", r'\texttt{payoff}', 4.75, 3,plot_params={"ec": "none"}))
...
pgm.add_node(daft.Node("actual_action", r"$b$", 4, 4, plot_params=action_color, observed=False))
...
pgm.render()
if AFTER:
    FILENAME = "simple_action_pair_PGM_AFTER"
else:
    FILENAME = "simple_action_pair_PGM_BEFORE"
